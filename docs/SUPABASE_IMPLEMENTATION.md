...
1. `create-order`: validates products and prices from the database, creates the pending order and items, initializes Paystack, and returns only the authorization URL/reference.
2. `paystack-webhook`: verifies Paystack’s signature, marks payment paid exactly once, writes the Paid event, and creates an admin notification. Never trust a browser success callback as payment confirmation.
...
- **Paystack:** the public key opens payment, but the secret key must remain server-side. A verified webhook—not the customer’s browser—changes `payment_status` to `paid`.
...
4. Add `VITE_SUPABASE_URL` and `VITE_SUPABASE_ANON_KEY`; keep service-role and Paystack secret keys only in server secrets.
