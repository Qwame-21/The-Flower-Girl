...
import { Check, LoaderCircle, LocateFixed, MapPin, X } from 'lucide-react';
      ...
      <section className="flow-modal checkout-modal" onScroll={event => { const node = event.currentTarget; const available = node.scrollHeight - node.clientHeight; setScrollState({ progress: available > 0 ? node.scrollTop / available : 0, visible: available > 8 }); }} onClick={(event) => event.stopPropagation()} aria-modal="true" role="dialog">
        ...
        {complete ? (
          <div className="flow-success payment-receipt">
            <Check size={28} />
            <span className="form-eyebrow">PAYMENT RETURN</span>
            <h2>{complete.status === 'paid' ? 'Your order is confirmed.' : complete.status === 'pending' ? 'Confirmation is taking longer.' : 'Confirming your payment.'}</h2>
