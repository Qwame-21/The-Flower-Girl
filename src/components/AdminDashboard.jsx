...
import { Bell, ChevronDown, ChevronRight, Download, LogOut, Search, ShoppingBag, X, LayoutDashboard, ClipboardList, WandSparkles, Package, ChartNoAxesColumnIncreasing, Users, Truck, BriefcaseBusiness, Settings, Images, MessageSquareText, Menu } from 'lucide-react';
...
export default function AdminDashboard({ onExit }) {
  ...
  const orderContent = <>
    <div className="factory-order-tabs">{['All', 'Unpaid', 'Packing', 'Delivery'].map(item => <button key={item} className={filter === item ? 'active' : ''} onClick={() => setFilter(item)}><span>{item === 'All' ? 'All orders' : item === 'Unpaid' ? 'Awaiting payment' : item === 'Packing' ? 'Preparing' : 'Ready for delivery'}</span><b>{item === 'All' ? orders.length : orders.filter(order => item === 'Unpaid' ? !order.status.payment : item === 'Packing' ? order.status.payment && !order.status.packaged : order.status.packaged && !order.status.delivered).length}</b></button>)}</div>
    <div className="factory-order-head"><div><span>ORDER WORKSPACE</span><h1>Needs attention</h1></div><label><Search size={16}/><input value={query} onChange={event=>setQuery(event.target.value)} placeholder="Search orders"/>{query&&<button onClick={()=>setQuery('')}><X size={14}/></button>}</label></div>
    <section className="factory-order-board">{visibleOrders.map(order => <article key={order.id} className={expanded === order.id ? 'expanded' : ''}><header><div><strong>{order.id}</strong><span>{order.name}</span></div><small><i/>{!order.status.payment?'Awaiting payment':order.status.packaged?'Ready for delivery':'Preparing'}</small></header><div className="factory-order-meta"><span><small>ORDER TYPE</small><b>{order.items}</b></span><span><small>PLACED</small><b>{order.time}</b></span></div><h2>{order.items.split('×')[0].trim()}</h2><p>{order.address} · GHS {order.total.toLocaleString()}</p>{expanded === order.id && <div className="factory-order-expanded"><p>{order.note}</p><div>{STAGES.map(stage=><button key={stage} className={order.status[stage]?'done':''} onClick={()=>toggleStage(order.id,stage)}>{stage}</button>)}</div></div>}<footer><button onClick={()=>setExpanded(current=>current===order.id?'':order.id)}>VIEW DETAILS</button><button onClick={()=>!order.status.payment?toggleStage(order.id,'payment'):!order.status.packaged?toggleStage(order.id,'packaged'):null}>{!order.status.payment?'CONFIRM PAYMENT':!order.status.packaged?'MARK READY':'ASSIGN RIDER'}</button></footer></article>)}</section>
  </>;
  ...
  return <main className="slay-admin-page factory-admin"><aside className="factory-rail"><button className="factory-rail-mark" onClick={()=>setTab('Overview')}>TGF</button><nav>{TABS.filter(item=>!['Log','Shop','Content'].includes(item)).map(item=>{const Icon=TAB_ICONS[item];return <button key={item} title={item} aria-label={item} className={tab===item?'active':''} onClick={()=>setTab(item)}><Icon size={17}/></button>})}</nav><button title="Storefront" aria-label="Storefront" onClick={onExit}><LogOut size={17}/></button></aside><div className="slay-admin-shell factory-admin-shell">
    <header className="slay-admin-heading"><div><button className="factory-mobile-trigger" onClick={() => setMobileMenuOpen(true)} aria-label="Open dashboard navigation"><Menu size={20} /></button><span className="slay-admin-title">{tab}</span><span className="slay-admin-mobile-title">{tab}</span><span className="slay-cloud-state"><i /> DEMO DATA</span><button className="slay-bell" onClick={() => setNoticeOpen(current => !current)} aria-label="Notifications"><Bell size={18} /><b>{awaiting.length}</b></button></div>{noticeOpen && <div className="slay-notice"><button onClick={() => setNoticeOpen(false)}><X size={14} /></button><small>OPERATIONS</small><strong>{awaiting.length} order awaiting delivery</strong><p>Review the delivery queue before assigning a rider.</p></div>}</header>
    {mobileMenuOpen && <div className="factory-mobile-drawer"><button className="factory-mobile-scrim" aria-label="Close dashboard navigation" onClick={() => setMobileMenuOpen(false)} /><aside><header><span>THE GIFTING FACTORY</span><button onClick={() => setMobileMenuOpen(false)} aria-label="Close dashboard navigation"><X size={19} /></button></header><nav>{TABS.map(item => { const Icon = TAB_ICONS[item]; return <button key={item} className={tab === item ? 'active' : ''} onClick={() => { setTab(item); setMobileMenuOpen(false); }}><Icon size={17} /><span>{item}</span><ChevronRight size={15} /></button>; })}</nav><button className="factory-mobile-storefront" onClick={onExit}><LogOut size={16} /> RETURN TO STOREFRONT</button></aside></div>}
