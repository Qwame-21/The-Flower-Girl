...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    const loadVerifiedData = async () => {
      ...
      const [ordersRes, requestsRes, appsRes, careersRes] = await Promise.all([
      ...
      if (active) {
        setAdminData(current => ({
          ...current,
          orders: ordersRes.data ? ordersRes.data.map(mapDatabaseOrder) : current.orders,
          requests: requestsRes.data ? requestsRes.data.map(mapDatabaseRequest) : current.requests,
          applications: appsRes.data ? appsRes.data.map(mapDatabaseApplication) : current.applications,
          careers: careersRes.data ? careersRes.data.map(mapDatabaseCareer) : current.careers,
        }));
      }
    };
    loadVerifiedData();
    const channel = supabase.channel('admin-dashboard-realtime')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'orders' }, loadVerifiedData)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'order_items' }, loadVerifiedData)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'order_events' }, loadVerifiedData)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'customer_requests' }, loadVerifiedData)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'career_applications' }, loadVerifiedData)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'careers' }, loadVerifiedData)
      .subscribe();
    return () => { active = false; supabase.removeChannel(channel); };
  }, []);

  const handleRequestStatusChange = async (requestId, newStatus) => {
    updateAdminCollection('requests', requests => requests.map(req => req.id === requestId ? { ...req, status: newStatus, updatedAt: new Date().toISOString() } : req));
    if (supabase) {
      await supabase.from('customer_requests').update({ status: newStatus }).eq('id', requestId);
    }
  };

  const handleSaveQuote = async (requestId, amount) => {
    const now = new Date().toISOString();
    updateAdminCollection('requests', requests => requests.map(req => req.id === requestId ? { ...req, confirmedQuote: amount, status: 'approved', approvedAt: now, updatedAt: now } : req));
    if (supabase) {
      await supabase.from('customer_requests').update({ confirmed_quote: amount, status: 'approved', approved_at: now }).eq('id', requestId);
    }
  };

  const handleConvertRequestToOrder = async (request) => {
    const seed = Date.now().toString().slice(-6);
    const tracking = `GF-REQ-${seed}`;
    const code = `REQ-${seed}`;
    const amount = Number(request.confirmedQuote || request.estimateHigh || 0);
    const newOrder = {
      tracking,
      code,
      ...
      delivery: request.deliveryAddress || request.note || 'Accra',
      customerNote: `Converted from Custom Request ${request.reference || ''}`,
      cardMessage: request.cardMessage || '',
      cardStyleNotes: request.cardStyleNotes || '',
      requestedDeliveryDate: request.preferredDate || request.date || null,
      total: amount,
      paymentMethod: 'Paystack',
      paymentStatus: 'pending',
      status: 'pending_payment',
      items: [{ id: 'custom-gift', name: `${request.service || 'Custom Gift'} (${request.quantity || 1} units)`, qty: request.quantity || 1, price: amount }],
    };

    addAdminRecord('orders', newOrder);
    handleRequestStatusChange(request.id, 'converted');
    ...
    if (supabase) {
      const { data: orderData } = await supabase.from('orders').insert({
        ...
        delivery_address: request.note || 'Accra',
        subtotal: amount,
        total: amount,
      ...
      if (orderData?.id) {
        await supabase.from('order_items').insert({
          ...
          item_name: `${request.service || 'Custom Gift'} (${request.quantity || 1} units)`,
          quantity: request.quantity || 1,
          unit_price: amount,
        });
      }

      await supabase.from('customer_requests').update({ status: 'converted', converted_at: new Date().toISOString() }).eq('id', request.id);
    }

    setActiveTabs(current => ({ ...current, Orders: 'All orders' }));
    selectNav('Orders');
  };

  const handleApplicationStatusChange = async (appId, newStatus) => {
    updateAdminCollection('applications', apps => apps.map(app => app.id === appId ? { ...app, status: newStatus, updatedAt: new Date().toISOString() } : app));
    if (supabase) {
      await supabase.from('career_applications').update({ status: newStatus }).eq('id', appId);
    }
  };

  const handleCareerStatusChange = async (careerId, newStatus) => {
    updateAdminCollection('careers', careers => careers.map(c => c.id === careerId ? { ...c, status: newStatus, updatedAt: new Date().toISOString() } : c));
    if (supabase) {
      await supabase.from('careers').update({ status: newStatus }).eq('id', careerId);
    }
  };
  ...
  const tabs = (() => {
    ...
    if (activeNav === 'Orders') return [['All orders', adminData.orders.filter(order => order.status !== 'completed').length, Package], ['Paid', adminData.orders.filter(order => order.paymentStatus === 'paid').length, ClipboardCheck], ['Preparing', adminData.orders.filter(order => order.status === 'packaging').length, Clock3], ['Ready', adminData.orders.filter(order => order.status === 'ready').length, Package], ['Log', adminData.orders.filter(order => order.status === 'completed').length, FileImage]];
    ...
    if (activeNav === 'Careers') return [['Applications', adminData.applications.filter(item => !['shortlisted', 'archived'].includes(item.status)).length, BriefcaseBusiness], ['Shortlist', adminData.applications.filter(item => item.status === 'shortlisted').length, ClipboardCheck], ['Archived', adminData.applications.filter(item => item.status === 'archived').length, Package], ['Open roles', adminData.careers.filter(role => role.status === 'open').length, BriefcaseBusiness], ['Drafts', adminData.careers.filter(role => role.status === 'draft').length, FileImage]];
  ...
  const filteredOrders = adminData.orders.filter(order => (activeTab === 'All orders' ? order.status !== 'completed' : activeTab === 'Paid' ? order.paymentStatus === 'paid' : activeTab === 'Preparing' ? order.status === 'packaging' : activeTab === 'Ready' ? order.status === 'ready' : activeTab === 'Log' ? order.status === 'completed' : true) && `${order.tracking} ${order.code || ''} ${order.customer} ${order.phone} ${order.delivery}`.toLowerCase().includes(orderQuery.toLowerCase()));
        ...
        <section id="admin-active-panel" role="tabpanel" aria-labelledby={`admin-tab-${activeNav}-${Math.max(0, tabIndex)}`} key={`${activeNav}-${activeTab}`} className="admin-foundation__canvas" aria-label={`${activeTab} workspace`}>
          {activeNav === 'Products' ? (
            ...
            <RequestsPage
              ...
              onConvertToOrder={handleConvertRequestToOrder}
              adminData={adminData}
            />
          ) : activeNav === 'Customers' ? (
            <CustomersPage
            ...
            <CareersPage
              ...
              onCareerStatusChange={handleCareerStatusChange}
              setSelectedItem={setSelectedItem}
            />
